(function(){
  ...
  function manipulateImage(type) {
    ...
    toggleButtonsAbledness(); // TURNS OFF BUTTON
    // INITIATE WORKER BY POSTING IMAGEDATA AND TYPE TO WORKER.JS
    imageWorker.postMessage({'imageData': imageData, 'type': type});

    // WHEN WORKER IS DONE, RETURN MESSAGE (OR IN THIS CASE THE IMG)
    imageWorker.onmessage = function(e) {
      toggleButtonsAbledness(); // TURNS BUTTON BACK ON
      var image = e.data;
      if (image) {
